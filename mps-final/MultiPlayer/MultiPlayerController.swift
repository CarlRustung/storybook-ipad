...
final class MultiPlayerController {
    ...
    let appDeclaration:String = StoryHandler.sharedInstance.multiPlayerID
    ...
    private init() {
        ...
        socket.on( "shareFlag" ) { data, ack in
            if let key = data[0] as? String {
                if let value = data[1] as? String {
                    StoryHandler.sharedInstance.flags[key] = value
                }
            }
        }
    ...
    //// SHARE FLAGS
    func shareFlag( key: String, value: String ) {
        socket.emit( "shareFlag", StoryHandler.sharedInstance.roomCode, key, value );
    }
